...
const useEnvFile = (sessionId: string) => {
  interface EnvFile {
    path: string
    raw: string
  }
  ...
  const selectFile = () => {
    getFilePathAndValue().then((data) => {
      env.path = data.path
      env.raw = data.value
      updateLocalData(sessionId, { path: env.path })
    })
  }
...
const { env, selectFile, readEnvFile, saveEnvFile } = useEnvFile(props.sessionId)
</script>
